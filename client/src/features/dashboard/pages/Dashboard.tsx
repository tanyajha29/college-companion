...
import { Home, Users, BookOpen, BarChart2, LogOut, GraduationCap, Shield } from "lucide-react";
...
export default function Dashboard() {
  ...
  // Guard against missing session mapping to the login fallback 
  if (!isAuthenticated || !role) {
    return (
      <div className="p-10 text-center text-red-600 dark:bg-gray-900 dark:text-red-400 min-h-screen">
        Session expired or not logged in. Please <a href="/login" className="underline font-bold">login</a>.
      </div>
    );
  }
          ...
          {navItems.map((item) => {
            if (item.roles.includes(role)) {
              ...
              return (
                <button
                  key={item.id}
                  className={`flex items-center gap-3 p-3 rounded-xl transition duration-200 ease-in-out text-base ${isActive
